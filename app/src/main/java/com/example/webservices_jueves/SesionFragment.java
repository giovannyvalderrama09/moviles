...
public class SesionFragment extends Fragment implements Response.Listener<JSONObject>,Response.ErrorListener {
    ...
    public void iniciar_sesion(){
        if (jetcorreo.getText().toString().isEmpty() || jetclave.getText().toString().isEmpty()){
            Toast.makeText(getContext(),"El correo y la clave son requeridos",Toast.LENGTH_SHORT).show();
            jetcorreo.requestFocus();
        }
        else{
            String url = "http://172.16.59.164:81/usuarios/sesion.php?correo="+jetcorreo.getText().toString()+"&clave="+jetclave.getText().toString();
            jrq = new JsonObjectRequest(Request.Method.GET,url,null,this,this);
            rq.add(jrq);
        }

       /* Intent registroventas=new Intent(getContext(),VentasActivity.class);
        startActivity(registroventas); */
    }
